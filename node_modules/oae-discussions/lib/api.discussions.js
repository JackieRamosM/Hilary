...
var createDiscussion = module.exports.createDiscussion = function(ctx, displayName, description, visibility, members, opts, callback) {
    console.log("in api.discussions.js");
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        DiscussionsAPI.Authz.resolveEffectiveDiscussionAccess(ctx, discussion, function(err, canView, canManage, canShare, canPost) {
            ...
            if (ctx.user()) {
                ...
                var tenantAlias = AuthzUtil.getResourceFromId(discussionId).tenantAlias;
                var body = ctx.user().id + ':' + discussionId;
                discussion.signature = Signature.createRollingSignature(tenantAlias, DiscussionsConstants.signature.EXPIRES, 20, body);
            }
...
/**
 * Validates a discussion signature
 *
 * @param  {Context}    ctx             The context of the current request
 * @param  {Validator}  validator       The validator
 * @param  {String}     discussionId    The id of the discussion to check
 * @param  {Object}     signature       The signature to check
 */
var validateSignature = module.exports.validateSignature = function(ctx, validator, discussionId, signature) {
    ...
    if (_.isObject(signature)) {
        ...
        var body = principalId + ':' + discussionId;
                ...
                AuthzAPI.updateRoles(discussionId, roleChanges, function(err) {
                    ...
                    _touch(discussion, function(err, touchedDiscussion) {
                        ...
                        _insertLibrary(principalIds, discussion, function(err) {
                            ...
                            DiscussionsAPI.emit(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, ctx, discussion, roleChanges, principalIds);
                            return callback();
                        });
                    });
                });
...
var setDiscussionPermissions = module.exports.setDiscussionPermissions = function(ctx, discussionId, permissionChanges, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        AuthzAPI.computeMemberRolesAfterChanges(discussionId, permissionChanges, function(err, membershipAfterChanges, newMemberIds, updatedMemberIds, removedMemberIds) {
            if (err) {
                return callback(err);
            }

            // Ensure the user is allowed to set the discussion permissions
            DiscussionsAPI.Authz.canSetDiscussionPermissions(ctx, discussion, newMemberIds, function(err, canSetPermissions, illegalMemberIds) {
                if (err) {
                    return callback(err);
                ...
                AuthzAPI.updateRoles(discussionId, permissionChanges, function(err) {
                    ...
                    _removeLibrary(removedMemberIds, discussion, function(err) {
                        ...
                        _touch(discussion, function(err, touchedDiscussion) {
                            ...
                            var libraryIdsToUpdate = _.union(updatedMemberIds, newMemberIds);
                            _updateLibrary(libraryIdsToUpdate, discussion, oldLastModified, function(err) {
                                if (err) {
                                    log().warn({
                                        'err': err,
                                        'principalIds': principalIds,
                                ...
                                DiscussionsAPI.emit(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, ctx, discussion, permissionChanges, newMemberIds, updatedMemberIds, removedMemberIds);
        ...
        DiscussionsAPI.Authz.canPostDiscussionMessage(ctx, discussion, function(err, canPost) {
            ...
            MessageBoxAPI.createMessage(discussionId, ctx.user().id, body, {'replyToCreated': replyToCreatedTimestamp}, function(err, message) {
                ...
                PrincipalsUtil.getPrincipal(ctx, ctx.user().id, function(err, createdBy) {
                    ...
                    DiscussionsAPI.emit(DiscussionsConstants.events.CREATED_DISCUSSION_MESSAGE, ctx, message, discussion);
