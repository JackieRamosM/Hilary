...
var purgeQueuesOnStartup = false;
var startupPurgeStatus = {};
    ...
    if (!connection) {
        ...
        if (mqConfig.purgeQueuesOnStartup === true) {
            if (process.env['NODE_ENV'] !== 'production') {
                ...
                log().warn('Attempted to set config.mq.purgeQueuesOnStartup to true when in production mode. Ignoring and not purging queues.');
        ...
        connection.on('error', function(err) {
            ...
            log().error({'err': err}, 'Error in the RabbitMQ connection. Reconnecting.');
 ...
 * @param  {Function}   callback            Standard callback method that will be executed when the exchange has been declared
 */
var declareExchange = module.exports.declareExchange = function(exchangeName, exchangeOptions, callback) {
    if (!exchangeName) {
        log().error({'exchangeName': exchangeName, 'err': new Error('Tried to declare an exchange without providing an exchange name')});
        return callback({'code': 400, 'msg': 'Tried to declare an exchange without providing an exchange name'});
    } else if (exchanges[exchangeName]) {
        log().error({'exchangeName': exchangeName, 'err': new Error('Tried to declare an exchange twice')});
 ...
 * @param  {Function}   callback            Standard callback method that will be executed when the exchange has been declared
 */
var declareQueue = module.exports.declareQueue = function(queueName, queueOptions, callback) {
    if (!queueName) {
        log().error({'queueName': queueName, 'queueOptions': queueOptions, 'err': new Error('Tried to declare a queue without providing a name')});
        return callback({'code': 400, 'msg': 'Tried to declare a queue without providing a name'});
    } else if (queues[queueName]) {
        log().error({'queueName': queueName, 'queueOptions': queueOptions, 'err': new Error('Tried to declare a queue twice')});
        return callback({'code': 400, 'msg': 'Tried to declare a queue twice'});
    }

    connection.queue(queueName, queueOptions, function(queue) {
        log().info({'queueName': queueName}, 'Created/Retrieved a RabbitMQ queue');
 ...
 */
var isQueueDeclared = module.exports.isQueueDeclared = function(queueName) {
 ...
 */
var _processBindQueue = function() {
    // If there is something to do and we're not already doing something we can do some work
    if (queuesToBind.length > 0 && !isWorking) {
        isWorking = true;

        var todo = queuesToBind.shift();
        todo.queue.bind(todo.exchangeName, todo.routingKey, function() {
            log().trace({'queueName': todo.queueName, 'exchangeName': todo.exchangeName, 'routingKey': todo.routingKey}, 'Bound a queue to an exchange');
 ...
 * @param  {Object}     callback.err    Standard error object in case something went wrong
 */
var bindQueueToExchange = module.exports.bindQueueToExchange = function(queueName, exchangeName, routingKey, callback) {
    if (!queues[queueName]) {
        log().error({'queueName': queueName, 'exchangeName': exchangeName, 'routingKey': routingKey, 'err': new Error('Tried to bind a non existing queue to an exchange, have you declared it first?')});
        return callback({'code': 400, 'msg': 'Tried to bind a non existing queue to an exchange, have you declared it first?'});
    } else if (!exchanges[exchangeName]) {
        log().error({'queueName': queueName, 'exchangeName': exchangeName, 'routingKey': routingKey, 'err': new Error('Tried to bind a queue to a non-existing exchange, have you declared it first?')});
        return callback({'code': 400, 'msg': 'Tried to bind a queue to a non-existing exchange, have you declared it first?'});
    } else if (!routingKey) {
        log().error({'queueName': queueName, 'exchangeName': exchangeName, 'routingKey': routingKey, 'err': new Error('Tried to bind a queue to an existing exchange without specifying a routing key')});
 ...
 * @param  {Object}     callback.err    Standard error object in case something went wrong
 */
var unbindQueueFromExchange = module.exports.unbindQueueFromExchange = function(queueName, exchangeName, routingKey, callback) {
    if (!queues[queueName]) {
        log().error({'queueName': queueName, 'exchangeName': exchangeName, 'routingKey': routingKey, 'err': new Error('Tried to unbind a non existing queue from an exchange, have you declared it first?')});
        return callback({'code': 400, 'msg': 'Tried to unbind a non existing queue from an exchange, have you declared it first?'});
    } else if (!exchanges[exchangeName]) {
        log().error({'queueName': queueName, 'exchangeName': exchangeName, 'routingKey': routingKey, 'err': new Error('Tried to unbind a queue from a non-existing exchange, have you declared it first?')});
        return callback({'code': 400, 'msg': 'Tried to unbind a queue from a non-existing exchange, have you declared it first?'});
    } else if (!routingKey) {
        log().error({'queueName': queueName, 'exchangeName': exchangeName, 'routingKey': routingKey, 'err': new Error('Tried to unbind a queue from an exchange without providing a routingKey')});
        return callback({'code': 400, 'msg': 'No routing key was specified'});
    }

    queues[queueName].queue.unbind(exchangeName, routingKey);
    log().trace({'queueName': queueName, 'exchangeName': exchangeName, 'routingKey': routingKey}, 'Unbound a queue from an exchange');

    // amqp's unbind method doesn't support a callback method.
    // To keep our unbind method consistent with the bind method
    // we do use a callback, although it's not strictly necessary.
    callback();
};
 ...
 * @param  {Function}   callback            Invoked when the listener has been subscribed
 * @param  {Object}     callback.err        An error that occurred, if any
 */
var subscribeQueue = module.exports.subscribeQueue = function(queueName, subscribeOptions, listener, callback) {
    callback = callback || function(err) {
        if (err) {
            log().warn({'err': err, 'queueName': queueName, 'subscribeOptions': subscribeOptions}, 'An error occurred while subscribing to a queue');
        }
    };

    if (!queues[queueName]) {
        log().error({'queueName': queueName, 'subscribeOptions': subscribeOptions, 'err': new Error('Tried to subscribe to an unknown queue')});
        return callback({'code': 400, 'msg': 'Tried to subscribe to an unknown queue'});
    }

    queues[queueName].queue.subscribe(subscribeOptions, function(data, headers, deliveryInfo, message) {
        log().trace({
            'queueName': queueName,
            'data': data,
            'headers': headers,
        ...
        if (deliveryInfo.redelivered) {
            ...
            MQ.submit(MqConstants.REDELIVER_EXCHANGE_NAME, MqConstants.REDELIVER_QUEUE_NAME, redeliveryData, MqConstants.REDELIVER_SUBMIT_OPTIONS, function(err) {
                if (err) {
                    log().warn({'err': err}, 'An error occurred delivering a redelivered message to the redelivery queue');
                }

                MQ.emit('storedRedelivery', queueName, data, headers, deliveryInfo, message);
            });
        ...
        MQ.emit('preHandle', queueName, data, headers, deliveryInfo, message);
        ...
        try {
            ...
            // Indicate that this server has finished processing the task
            _decrementProcessingTask(deliveryKey, deliveryInfo);
            MQ.emit('postHandle', err, queueName, data, headers, deliveryInfo);
        }
    }).addCallback(function(ok) {
        if (!ok) {
            log().error({'queueName': queueName, 'err': new Error('Error binding worker for queue')});
            return unsubscribeQueue(queueName, function(err) {
 ...
 * @param  {Object}    callback.err    An error that occurred, if any
 */
var unsubscribeQueue = module.exports.unsubscribeQueue = function(queueName, callback) {
    var queue = queues[queueName];
    if (!queue || !queue.queue) {
        log().warn({'queueName': queueName, 'err': new Error('Attempted to unbind listener from non-existant job queue. Ignoring')});
        return callback();
    }
 ...
 *
 * @param  {Function}   callback    Invoked when all handlers have been unbound
 * @api private
 */
var _unsubscribeAll = function(callback) {
    var queuesUnbound = 0;
    var queueNames = _.keys(queues);
    if (queueNames.length > 0) {
        _.each(queueNames, function(queueName) {
            log().info({'queueName': queueName}, 'Attempting to unbind a queue');
            unsubscribeQueue(queueName, function(err) {
                queuesUnbound++;

                if (!err) {
                    log().info({'queueName': queueName}, 'Successfully unbound a queue');
                }
 ...
 */
var submit = module.exports.submit = function(exchangeName, routingKey, data, options, callback) {
console.log("en submit &&&&&&&&&&&&&&&&&&&&&&&&");
    options = options || {};
    callback = callback || function() {};

    if (!exchanges[exchangeName]) {
        log().error({'exchangeName': exchangeName, 'routingKey': routingKey, 'err': new Error('Tried to submit a message to an unknown exchange')});
        return callback({'code': 400, 'msg': 'Tried to submit a message to an unknown exchange'});
	console.log("error 1");
    } else if (!routingKey) {
        log().error({'exchangeName': exchangeName, 'routingKey': routingKey, 'err': new Error('Tried to submit a message without specifying a routingKey')});
        return callback({'code': 400, 'msg': 'Tried to submit a message without specifying a routingKey'});
	console.log("erro 2");
    }

    MQ.emit('preSubmit', routingKey);
	console.log(routingKey, "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
    exchanges[exchangeName].publish(routingKey, data, options, function(err) {
        console.log(exchangeName);	

	if (err) {
            log().error({'exchangeName': exchangeName, 'routingKey': routingKey, 'data': data, 'options': options}, 'Failed to submit a message to an exchange');
            return callback(err);
		console.log("error 3");
        }
    ...
    // If `confirm` is not set, node-amqp will *NOT* call our callback
    if (exchanges[exchangeName].options.confirm !== true) {
        return callback();
	console.log("error 4");
    }
};
 ...
 */
var getBoundQueueNames = module.exports.getBoundQueueNames = function() {
    return _.chain(queues).keys().filter(function(queueName) { return !_.isUndefined(queues[queueName].consumerTag); }).value();
};
 ...
 */
var purge = module.exports.purge = function(queueName, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({'err': err, 'queueName': queueName}, 'Error purging queue.');
        }
    };

    if (!queues[queueName]) {
        log().error({'queueName': queueName, 'err': new Error('Tried purging an unknown queue')});
        return callback({'code': 400, 'msg': 'Tried purging an unknown queue'});
    }

    log().info({'queueName': queueName}, 'Purging queue');
    MQ.emit('prePurge', queueName);
    queues[queueName].queue.purge().addCallback(function(data) {
        if (!data) {
            log().error({'err': err, 'queueName': queueName}, 'Error purging queue');
 ...
 *
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occurred creating the retry queue, if any
 * @api private
 */
var _createRedeliveryQueue = function(callback) {
    ...
    // being acknowledged
    declareExchange(MqConstants.REDELIVER_EXCHANGE_NAME, MqConstants.REDELIVER_EXCHANGE_OPTIONS, function(err) {
        if (err) {
            return callback(err);
        }

        declareQueue(MqConstants.REDELIVER_QUEUE_NAME, MqConstants.REDELIVER_QUEUE_OPTIONS, function(err) {
            if (err) {
                return callback(err);
            }

            return bindQueueToExchange(MqConstants.REDELIVER_QUEUE_NAME, MqConstants.REDELIVER_EXCHANGE_NAME, MqConstants.REDELIVER_QUEUE_NAME, callback);
        });
    });
};
...
var _incrementProcessingTask = function(deliveryKey, data, deliveryInfo) {
    if (numMessagesInProcessing >= MAX_NUM_MESSAGES_IN_PROCESSING) {
        _dumpProcessingMessages('Reached maximum number of concurrent messages allowed in processing (' +
            MAX_NUM_MESSAGES_IN_PROCESSING + '), this probably means there were many messages received that were never acknowledged.' +
