...
var insert = module.exports.insert = function(indexName, libraryVisibilities, resourceId, rank, callback) {
    ...
    return _insert(indexName, indexEntries, callback);
};
    ...
    _.each(visibilityMasks, function(mask, bucketName) {
        _.each(libraryVisibilities, function(visibility, libraryId) {
            queries.push({
                ...
                'parameters': [oldValueKey, _createBucketKey(indexName, libraryId, bucketName)]
            });
        });
    });
        ...
        _.each(visibilityMasks, function(mask, bucketName) {
            if (_.contains(mask, visibility)) {
                ...
                queries.push({
                    ...
                    'parameters': [newValueKey, value, _createBucketKey(indexName, libraryId, bucketName)]
                });
            }
        });
    });
...
var remove = module.exports.remove = function(indexName, libraryIds, resourceId, rank, callback) {
    ...
    _.each(visibilityMasks, function(mask, bucketName) {
        _.each(libraryIds, function(libraryId) {
            queries.push({
                'query': 'DELETE ? FROM LibraryIndex WHERE bucketKey = ?',
                'parameters': [valueKey, _createBucketKey(indexName, libraryId, bucketName)]
            });
        });
    });
...
var list = module.exports.list = function(indexName, libraryId, visibility, opts, callback) {
	...
	console.log(indexName, libraryId, visibility,opts);
	//meeting:meeting u:oae:g1e7-lUXah public { start: 0, limit: 12 }
	//discussions:discussions u:oae:g1e7-lUXah private { start: undefined, limit: 12 }
    ...
    _query(indexName, libraryId, visibility, {'start': opts.start, 'limit': opts.limit, 'rebuildIfNecessary': true}, function(err, valueKeys, nextToken) {
        ...
        if (valueKeysToDelete.length > 0) {
            ...
            valueKeysToDelete.push(_createBucketKey(indexName, libraryId, visibility));
...
var purge = module.exports.purge = function(indexName, libraryId, callback) {
    ...
    Cassandra.runBatchQuery(purgeIndexQueries, 'QUORUM', callback);
};
 ...
 * @param  {Object}     [opts]                  Optional parameters for the query
 * @param  {String}     [opts.start]            The starting point from which to start fetching library items. Default: Starts from the highest-ranked item
 * @param  {Number}     [opts.limit]            The maximum number of library items to return. Default: 25
 * @param  {Boolean}    [opts.rebuildIfNeeded]  Whether or not to rebuild the library index if it is detected that it is not "fresh" (i.e., it has not been rebuilt since the last purge)
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred if any
...
var _query = function(indexName, libraryId, visibility, opts, callback) {
    ...
    Cassandra.runPagedColumnQuery('LibraryIndex', 'bucketKey', bucketKey, opts.start, internalLimit, {'reversed': true}, function(err, columns, nextToken) {
        if (err) {
            return callback(err);
        } else if (_isStaleLibraryIndex(opts.start, internalLimit, columns)) {
        ...
        var result = _adjustColumnsForSlugs(opts.start, opts.limit, columns, nextToken);
...
var _rebuild = function(indexName, libraryId, callback) {
    purge(indexName, libraryId, function(err) {
        ...
        var seedLibraryQueries = _.map(visibilityMasks, function(mask, visibility) {
            return {
                'query': 'UPDATE LibraryIndex SET ? = ?, ? = ? WHERE bucketKey = ?',
                'parameters': [SLUG_HIGH, '1', SLUG_LOW, '1', _createBucketKey(indexName, libraryId, visibility)]
            };
        });
        ...
        Cassandra.runBatchQuery(seedLibraryQueries, 'QUORUM', function(err) {
            if (err) {
                return callback(err);
            }

            // Build the index after it has been destroyed and seeded
            _build(indexName, libraryId, callback);
        });
    });
};
    ...
    _.each(indexEntries, function(indexEntry) {
        ...
        _.each(visibilityMasks, function(mask, visibility) {
            if (_.contains(mask, indexEntry.visibility)) {
                queries.push({
                    'query': 'UPDATE LibraryIndex SET ? = ? WHERE bucketKey = ?',
                    'parameters': [valueKey, value, _createBucketKey(indexName, indexEntry.libraryId, visibility)]
                });
            }
        });
    });
 ...
 *
 * @param  {String}     start       The start parameter that was specified to result in this `columns` result
 * @param  {Number}     limit       The limit that was used for this `columns` result
 * @param  {Column[]}   columns     The cassandra columns that represents the result of the library index query
 * @return {Boolean}                `true` if this row indicates that the library index is stale, `false` otherwise
 * @api private
 */
var _isStaleLibraryIndex = function(start, limit, columns) {
    var slugHighColumn = _.findWhere(columns, {'name': SLUG_HIGH});
    var slugLowColumn = _.findWhere(columns, {'name': SLUG_LOW});

    if (!start && !slugHighColumn) {
        // If we didn't supply a start parameter and did not get the upper bound limiter, we need to refresh the index because it indicates
        // this index has been purged and not yet rebuilt
        return true;
    } else if (columns.length < limit && !slugLowColumn) {
        // If we exhausted the entries and the last entry wasn't the low-bound slug, then we have a purged index and need to rebuild it
        return true;
    }
...
/**
 * Given a list of columns that were queried from a library index, adjust it for the fact that:
 *
 ...
 * @param  {Column[]}   columns     The list of cassandra columns that were returned from the query
 ...
 * @api private
 */
var _adjustColumnsForSlugs = function(start, limit, columns, nextToken) {
    var result = {
        'keys': [],
        'nextToken': nextToken
    };
    ...
    columns.forEach(function(column) {
        ...
        if (value === '1') {
            ...
            } else if (name !== SLUG_HIGH) {
                // We have found an actual column key, push it into the results
