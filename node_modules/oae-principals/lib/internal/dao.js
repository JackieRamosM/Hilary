...
var getPrincipals = module.exports.getPrincipals = function(principalIds, fields, callback) {
    ...
    if (fields) {
        ...
        query = 'SELECT ' + placeholders.join(',') + ' FROM Principals USING CONSISTENCY QUORUM WHERE principalId IN (?)';
    } else {
        query = 'SELECT * FROM Principals USING CONSISTENCY QUORUM WHERE principalId IN (?)';
    }
...
var acceptTermsAndConditions = module.exports.acceptTermsAndConditions = function(userId, callback) {
    Cassandra.runQuery('UPDATE Principals USING CONSISTENCY QUORUM SET ? = ? WHERE principalId = ?', ['acceptedTC', Date.now(), userId], function(err) {
        if (err) {
            return callback(err);
        }
