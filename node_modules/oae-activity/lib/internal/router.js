...
var routeActivity = module.exports.routeActivity = function(activitySeed, callback) {
    ...
    _produceAllEntities(activitySeed, function(err, actor, object, target) {
        if (err) {
            log().error({'err': err, 'activitySeed': activitySeed}, 'An error occurred when producing the entities for an activity seed');
            return callback(err);
        }

        // Produce the routes from the actor, object and target entities
        _produceAllRoutes(activitySeed, actor, object, target, function(err, routes) {
            if (err) {
                log().error({'err': err, 'activitySeed': activitySeed}, 'An error occurred when producing the routes for an activity seed');
                return callback(err);
            }

            // If no routes were generated, we're done
            if (_.isEmpty(routes)) {
                return callback();
            }
            ...
            _.each(routes, function(route) {
				...
				console.log("t1", activitySeed.activityType, activityId);
                if (actor) {
                ...
                var activityStreamId = ActivityUtil.createActivityStreamId(route.resourceId, route.streamType);
                allActivities[activityStreamId] = {
                ...
                if (!route.transient) {
                    persistedActivities[activityStreamId] = activity;
                }
            });
            ...
            }, 'Activities to be persisted/emitted');

		console.log("antes de activity emiter en router -----------");
            // Emit an event for the routed activities
            ActivityEmitter.emit(ActivityConstants.events.ROUTED_ACTIVITIES, activitySeed, allActivities);
		console.log("despues de activity emiter en router -----------");
            // Queue the activity entities to be collected and aggregated
            _queueActivities(activitySeed, persistedActivities, function(err, buckets) {
                if (err) {
                    return callback(err);
                }
                ...
                Telemetry.incr('routed.count', _.keys(persistedActivities).length);
...
var _produceAllEntities = function(activitySeed, callback) {
    _produceEntity(activitySeed.actorResource, function(err, actor) {
        if (err) {
            return callback(err);
        }

        _produceEntity(activitySeed.objectResource, function(err, object) {
            if (err) {
                return callback(err);
            }

            _produceEntity(activitySeed.targetResource, function(err, target) {
                if (err) {
                    return callback(err);
                }
 ...
 * @api private
 */
var _produceEntity = function(resource, callback) {
    if (!resource) {
        return callback();
    }

    // Find the producer for this resource type. If there isn't one registered, we fall back to the default producer
    var activityEntityType = ActivityRegistry.getRegisteredActivityEntityTypes()[resource.resourceType] || {};
    var producer = activityEntityType.producer || _defaultActivityEntityProducer;

    producer(resource, function(err, entity) {
        if (err) {
            return callback(err);
        }
...
var _produceAllRoutes = function(activitySeed, actor, object, target, callback) {
    ...
    var associationsSession = new ActivityModel.AssociationsSession(ActivityRegistry.getRegisteredActivityEntityAssociations(), actor, object, target);
    var activityConfig = ActivityRegistry.getRegisteredActivityTypes()[activitySeed.activityType];
    ...
    var actorRouteConfig = _getRouterConfigForObjectType(activityConfig, ActivityConstants.entityTypes.ACTOR);
    var objectRouteConfig = _getRouterConfigForObjectType(activityConfig, ActivityConstants.entityTypes.OBJECT);
    var targetRouteConfig = _getRouterConfigForObjectType(activityConfig, ActivityConstants.entityTypes.TARGET);
    ...
    var associationsContexts = {
        'actor': associationsSession.createAssociationsContext(actor.objectType, actor[ActivityConstants.properties.OAE_ID]),
        'object': (object) ? associationsSession.createAssociationsContext(object.objectType, object[ActivityConstants.properties.OAE_ID]) : null,
        'target': (target) ? associationsSession.createAssociationsContext(target.objectType, target[ActivityConstants.properties.OAE_ID]) : null
    };

    // Route the actor entity
    _produceRoutes(associationsContexts.actor, actor, actorRouteConfig, function(err, actorRoutes) {
        if (err) {
            return callback(err);
        }

        // Route the object entity
        _produceRoutes(associationsContexts.object, object, objectRouteConfig, function(err, objectRoutes) {
            if (err) {
                return callback(err);
            }

            // Route the target entity
            _produceRoutes(associationsContexts.target, target, targetRouteConfig, function(err, targetRoutes) {
                if (err) {
                    return callback(err);
                }

                // Determine the propagation rules for the actor
                _producePropagation(associationsContexts.actor, actor, function(err, actorPropagationRules) {
                    if (err) {
                        return callback(err);
                    }

                    // Determine the propagation rules for the object
                    _producePropagation(associationsContexts.object, object, function(err, objectPropagationRules) {
                        if (err) {
                            return callback(err);
                        }

                        // Determine the propagation rules for the target
                        _producePropagation(associationsContexts.target, target, function(err, targetPropagationRules) {
                            if (err) {
                                return callback(err);
                            }
                            ...
                            var allRoutes = _.chain(actorRoutes).union(objectRoutes, targetRoutes)
                                            .compact()
                                            .filter(function(route) {
                                                if (route.streamType === 'notification') {
                                                    // If this is a notification route, we can only accept users other than the actor
                                                    return (AuthzUtil.isUserId(route.resourceId) && route.resourceId !== actor[ActivityConstants.properties.OAE_ID]);
                                                } else {
                                                    // This route is not a notification, we need to include it so we can apply propagation on it
                                                    return true;
                                                }
                                                return true;
                                            })
                            ...
                            _applyPropagations(ActivityConstants.entityTypes.ACTOR, actorPropagationRules, allRoutes, associationsContexts, actor, actorRoutes, function(err, includedRoutes) {
                                if (err) {
                                    return callback(err);
                                } else if (_.isEmpty(includedRoutes)) {
                                    // No routes survived the actor propagation, simply return the empty array
                                    return callback(null, []);
                                }

                                // Further filter routes down by object propagation
                                _applyPropagations(ActivityConstants.entityTypes.OBJECT, objectPropagationRules, includedRoutes, associationsContexts, object, objectRoutes, function(err, includedRoutes) {
                                    if (err) {
                                        return callback(err);
                                    } else if (_.isEmpty(includedRoutes)) {
                                    ...
                                    _applyPropagations(ActivityConstants.entityTypes.TARGET, targetPropagationRules, includedRoutes, associationsContexts, target, targetRoutes, function(err, includedRoutes) {
                                        if (err) {
                                            return callback(err);
                                        }
 ...
 * @api private
 */
var _applyPropagations = function(objectType, propagations, routes, associationsContexts, entity, entityRoutes, callback, _includeRoutes, _excludeRoutes) {
    if (!entity) {
    ...
    var propagation = propagations.shift();
    _applyPropagation(objectType, propagation, _excludeRoutes, associationsContexts, entity, entityRoutes, function(err, includeRoutes, excludeRoutes) {
        if (err) {
            log().warn({'err': err, 'propagation': propagation, 'entity': entity}, 'There was an error applying a propagation rule for an entity');
        } else {
        ...
        if (excludeRoutes.length === 0) {
            ...
            return _applyPropagations(objectType, propagations, routes, associationsContexts, entity, entityRoutes, callback, _includeRoutes, _excludeRoutes);
        }
    });
};
 ...
 * @api private
 */
var _applyPropagation = function(objectType, propagation, routes, associationsContexts, entity, entityRoutes, callback) {
    entityRoutes = entityRoutes || [];

    var entityId = entity[ActivityConstants.properties.OAE_ID];
    var entityTenantAlias = AuthzUtil.getResourceFromId(entityId).tenantAlias;
     ...
     */
    var _applyAssociationsCtx = function(associationsCtx, associationName, callback) {
        associationsCtx.get(associationName, function(err, associations) {
            if (err) {
                log().warn({'err': err, 'association': associationName}, 'An error occurred while applying associations');
            }

            // Index the entity associations into a hash to make the next loop O(n) instead of O(n^2)
            var associationRoutesHash = {};
            _.each(associations, function(associationRoute) { associationRoutesHash[associationRoute] = true; });
    ...
    if (propagation.type === ActivityConstants.entityPropagation.ALL) {
        ...
        return callback(null, routes, []);
    } else if (propagation.type === ActivityConstants.entityPropagation.TENANT) {
    ...
    } else if (propagation.type === ActivityConstants.entityPropagation.INTERACTING_TENANTS) {
        ...
        return callback(null, includedRoutes, excludedRoutes);
    } else if (propagation.type === ActivityConstants.entityPropagation.ROUTES) {
        // Index the entity routes into a hash to make the next loop O(n) instead of O(n^2)
        var entityRoutesHash = {};
        _.each(entityRoutes, function(entityRoute) { entityRoutesHash[entityRoute.resourceId] = true; });
        ...
        return callback(null, includedRoutes, excludedRoutes);
    } else if (propagation.type === ActivityConstants.entityPropagation.SELF) {
        // Apply the "self" association of the item
        _applyAssociationsCtx(associationsContexts[objectType], 'self', function(err) {
            if (err) {
                return callback(err);
            }
        ...
        _applyAssociationsCtx(associationsContexts[objectType], propagation.association, function(err) {
            if (err) {
                return callback(err);
            }
        ...
        var externalAssociationsCtx = associationsContexts[propagation.objectType];
        ...
        _applyAssociationsCtx(externalAssociationsCtx, propagation.association, function(err) {
            if (err) {
                return callback(err);
            }
 ...
 * @api private
 */
var _producePropagation = function(associationsCtx, entity, callback) {
    if (!entity) {
        return callback();
    }

    var entityTypeConfig = ActivityRegistry.getRegisteredActivityEntityTypes()[entity.objectType];
 ...
 * @api private
 */
var _produceRoutes = function(associationsCtx, entity, routeConfig, callback) {
     ...
     */
    var routeStreamAssociations = function() {
        if (routeConfig.length === 0) {
            log().trace({'entity': entity, 'routes': routes}, 'Generated routes for activity entity');
            return callback(null, routes);
        }

        var stream = routeConfig.pop();
        _routeAssociations(associationsCtx, stream.associationNames.slice(), function(err, streamRoutes) {
            if (err) {
                return callback(err);
            }

            // Collect the routes for this stream
            _.each(streamRoutes, function(streamRoute) {
                routes.push({
 ...
 * @api private
 */
var _routeAssociations = function(associationsCtx, associationNames, callback, _routes) {
    _routes = _routes || [];
    if (!associationNames || associationNames.length === 0) {
        return callback(null, _routes);
    }

    var associationName = associationNames.shift();
    associationsCtx.get(associationName, function(err, associations) {
        if (err) {
            log().warn({'err': err}, 'Error fetching association "%s"', associationName);
 ...
 */
var _getRouterConfigForObjectType = function(activityConfig, objectType) {
    var routerConfig = [];
    if (!activityConfig || !activityConfig.streams) {
        return routerConfig;
    }

    _.each(activityConfig.streams, function(streamConfig, streamType) {
 ...
 * @api private
 */
var _defaultActivityEntityProducer = function(resource, callback) {
...
var _queueActivities = function(activitySeed, routedActivities, callback) {
    ...
    ActivityDAO.saveQueuedActivities(activityBuckets, function(err) {
        if (err) {
            log().error({'err': err, 'activitySeed': activitySeed}, 'Could not save the queued activities for an activity seed');
            return callback(err);
        }
