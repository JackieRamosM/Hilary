 ...
 */
var ActivityStream = module.exports.ActivityStream = function(activities, nextToken) {
    return {'items': activities, 'nextToken': nextToken};
};
...
var AssociationsSession = module.exports.AssociationsSession = function(registeredAssociations, actor, object, target) {
    ...
    if (object) {
        ...
        _knownEntities[object.objectType][object[ActivityConstants.properties.OAE_ID]] = _knownEntities[object.objectType][object[ActivityConstants.properties.OAE_ID]] || object;
    }

    if (target) {
        _knownEntities[target.objectType] = _knownEntities[target.objectType] || {};
        _knownEntities[target.objectType][target[ActivityConstants.properties.OAE_ID]] = _knownEntities[target.objectType][target[ActivityConstants.properties.OAE_ID]] || target;
    }
    ...
    associationsSession.getByEntityId = function(entityType, entityId, associationName, callback) {
        ...
        var cachedAssociation = (_associationsCache[entityType] && _associationsCache[entityType][entityId] && _associationsCache[entityType][entityId][associationName]);
        ...
        var associationFunction = (registeredAssociations[entityType] && registeredAssociations[entityType][associationName]);
        if (!_.isFunction(associationFunction)) {
            return callback();
        }

        // Ensure the entity is actually a member of the session (i.e., is actor, object or target)
        var entity = (_knownEntities[entityType] && _knownEntities[entityType][entityId]);
        if (!entity) {
            return callback();
        }

        // Invoke the association function with the entity identified by type/id in context
        associationFunction(associationsSession.createAssociationsContext(entityType, entityId), entity, function(err, association) {
            if (err) {
                return callback(err);
            } else if (!association) {
    ...
    associationsSession.createAssociationsContext = function(entityType, entityId) {
         ...
         */
        var getSession = associationsContext.getSession = function() {
         ...
         * @param  {Object}     callback.association    The association. Usually a list of strings indicating ids, however can really be any value. Note that it cannot be used for routing purposes if it does not return an array of strings
         */
        var get = associationsContext.get = function(associationName, callback) {
            return getSession().getByEntityId(entityType, entityId, associationName, callback);
        };
