 ...
 *  * `routedActivities(activities)` - Indicates activities have been routed to transient streams. The individual transient activities are provided, these will be discarded after this event
 */
...
var refreshConfiguration = module.exports.refreshConfiguration = function(config, callback) {
	...
	console.log("llamando a taskqueu.bind desde actitivy api-----------------");
        return TaskQueue.bind(ActivityConstants.mq.TASK_ACTIVITY, ActivityRouter.routeActivity, {'subscribe': {'prefetchCount': config.maxConcurrentRouters}}, callback);
 ...
 *
 * @param  {String}     activityStreamType                          The name of the stream that will be registered. ex: `activity`, `notification`, `message`, ...
 * @param  {Object}     options                                     The options for this stream
 * @param  {Boolean}    options.transient                           If `true`, activities in this stream will be persisted to the database
 ...
 */
var registerActivityStreamType = module.exports.registerActivityStreamType = function(activityStreamType, options) {
 ...
 */
var getRegisteredActivityStreamType = module.exports.getRegisteredActivityStreamType = function(activtyStreamType) {
 ...
 * as `content` items or `discussion`s by adding the `self` association. This is how push notifications for those kind of resources work.
 *
...
var registerActivityType = module.exports.registerActivityType = function(activityType, options) {
	console.log("+++++++++++++++ en registerActivityType================================== \n\n\n=============================");
...
/**
 * Get the activity stream by the specified `resourceId` and `activtyStreamType`.
 *
 * @param  {Context}            ctx                     The context of the current session
 * @param  {String}             resourceId              The ID of the resource for which we want to retrieve a stream
 * @param  {String}             activtyStreamType       The name of the activity stream that should be retrieved
...
var postActivity = module.exports.postActivity = function(ctx, activitySeed, callback) {
	...
	console.log("Activities not enabled");
        return callback();
    }
    ...
    if (activitySeed) {
        ...
        if (activitySeed.actorResource) {
			console.log("Validating actor resource");
        ...
        if (activitySeed.objectResource) {
			console.log("Validating object resource");
        ...
        if (activitySeed.targetResource) {
			console.log("Validating target resource");
    ...
    if (validator.hasErrors()) {
		...
		console.log("Before submitting to taskqueue");
 ...
 * @api private
 */
var _getActivityStream = function(ctx, activityStreamId, start, limit, transformerType, callback) {
    ActivityDAO.getActivities(activityStreamId, start, limit, function(err, activities, nextToken) {
        if (err) {
            return callback(err);
        }

        ActivityTransformer.transformActivities(ctx, activities, transformerType, function(err) {
            if (err) {
                return callback(err);
            }
